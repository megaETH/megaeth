...
import "./interfaces/tokens/IMEGAToken.sol";
import "./interfaces/tokens/IESMEGAToken.sol";
import "./interfaces/IESMEGATokenUsage.sol";

/*
 * ESMEGA is MEGA's escrowed governance token obtainable by converting MEGA to it
 * It's non-transferable, except from/to whitelisted addresses
 * It can be converted back to MEGA through a vesting process
 * This contract is made to receive ESMEGA deposits from users in order to allocate them to Usages (plugins) contracts
 */

contract ESMEGAToken is Ownable, ReentrancyGuard, ERC20("MEGA Escrowed Token", "ESMEGA"), IESMEGAToken {
  using Address for address;
  using SafeMath for uint256;
  using EnumerableSet for EnumerableSet.AddressSet;
  using SafeERC20 for IMEGAToken;

  struct ESMEGABalance {
    uint256 allocatedAmount; // Amount of ESMEGA allocated to a Usage
    uint256 redeemingAmount; // Total amount of ESMEGA  currently being redeemed
  }

  struct RedeemInfo {
    uint256 megaAmount; // MEGA amount to receive when vesting has ended
    uint256 ESMEGAAmount; // ESMEGA amount to redeem
    uint256 endTime;
    IESMEGATokenUsage dividendsAddress;
    uint256 dividendsAllocation; // Share of redeeming ESMEGA to allocate to the Dividends Usage contract
  }

  IMEGAToken public immutable megaToken; // MEGA token to convert to/from
  IESMEGATokenUsage public dividendsAddress; // MEGA dividends contract

  EnumerableSet.AddressSet private _transferWhitelist; // addresses allowed to send/receive ESMEGA

  mapping(address => mapping(address => uint256)) public usageApprovals; // Usage approvals to allocate ESMEGA
  mapping(address => mapping(address => uint256)) public override usageAllocations; // Active ESMEGA allocations to usages

  uint256 public constant MAX_DEALLOCATION_FEE = 200; // 2%
  mapping(address => uint256) public usagesDeallocationFee; // Fee paid when deallocating ESMEGA
  ...
  // Adjusted dividends rewards for redeeming ESMEGA
  uint256 public redeemDividendsAdjustment = 0; // 50%

  mapping(address => ESMEGABalance) public ESMEGABalances; // User's ESMEGA balances
  mapping(address => RedeemInfo[]) public userRedeems; // User's redeeming instances


  constructor(IMEGAToken megaToken_) {
    megaToken = megaToken_;
  ...
  event Redeem(address indexed userAddress, uint256 ESMEGAAmount, uint256 megaAmount, uint256 duration);
  event FinalizeRedeem(address indexed userAddress, uint256 ESMEGAAmount, uint256 megaAmount);
  event CancelRedeem(address indexed userAddress, uint256 ESMEGAAmount);
  ...
  /*
   * @dev Returns user's ESMEGA balances
   */
  function getESMEGABalance(address userAddress) external view returns (uint256 allocatedAmount, uint256 redeemingAmount) {
    ESMEGABalance storage balance = ESMEGABalances[userAddress];
    return (balance.allocatedAmount, balance.redeemingAmount);
  }

  /*
   * @dev returns redeemable mega for "amount" of ESMEGA vested for "duration" seconds
   */
  function getmegaByVestingDuration(uint256 amount, uint256 duration) public view returns (uint256) {
   ...
   */
  function getUserRedeem(address userAddress, uint256 redeemIndex) external view validateRedeem(userAddress, redeemIndex) returns (uint256 megaAmount, uint256 ESMEGAAmount, uint256 endTime, address dividendsContract, uint256 dividendsAllocation) {
    RedeemInfo storage _redeem = userRedeems[userAddress][redeemIndex];
    return (_redeem.megaAmount, _redeem.ESMEGAAmount, _redeem.endTime, address(_redeem.dividendsAddress), _redeem.dividendsAllocation);
  }

  /**
   * @dev returns approved ESMEGA to allocate from "userAddress" to "usageAddress"
   */
  ...
  /**
   * @dev returns allocated ESMEGA from "userAddress" to "usageAddress"
   */
  ...
  /**
   * @dev returns if "account" is allowed to send/receive ESMEGA
   */
   ...
   * Must only be called by owner
   */
  function updateDividendsAddress(IESMEGATokenUsage dividendsAddress_) external onlyOwner {
  ...
  function updateTransferWhitelist(address account, bool add) external onlyOwner {
    require(account != address(this), "updateTransferWhitelist: Cannot remove ESMEGA from whitelist");
  ...
  /**
   * @dev Approves "usage" address to get allocations up to "amount" of ESMEGA from msg.sender
   */
  function approveUsage(IESMEGATokenUsage usage, uint256 amount) external nonReentrant {
  ...
  /**
   * @dev Convert caller's "amount" of mega to ESMEGA
   */
  ...
  /**
   * @dev Convert caller's "amount" of mega to ESMEGA to "to" address
   */
  ...
  /**
   * @dev Initiates redeem process (ESMEGA to mega)
   *
   * Handles dividends' compensation allocation during the vesting process if needed
   */
  function redeem(uint256 ESMEGAAmount, uint256 duration) external nonReentrant {
    require(ESMEGAAmount > 0, "redeem: ESMEGAAmount cannot be null");
    require(duration >= minRedeemDuration, "redeem: duration too low");

    _transfer(msg.sender, address(this), ESMEGAAmount);
    ESMEGABalance storage balance = ESMEGABalances[msg.sender];

    // get corresponding mega amount
    uint256 megaAmount = getmegaByVestingDuration(ESMEGAAmount, duration);
    emit Redeem(msg.sender, ESMEGAAmount, megaAmount, duration);

    // if redeeming is not immediate, go through vesting process
    if(duration > 0) {
      // add to SBT total
      balance.redeemingAmount = balance.redeemingAmount.add(ESMEGAAmount);

      // handle dividends during the vesting process
      uint256 dividendsAllocation = ESMEGAAmount.mul(redeemDividendsAdjustment).div(100);
      ...
      // add redeeming entry
      userRedeems[msg.sender].push(RedeemInfo(megaAmount, ESMEGAAmount, _currentBlockTimestamp().add(duration), dividendsAddress, dividendsAllocation));
    } else {
      // immediately redeem for mega
      _finalizeRedeem(msg.sender, ESMEGAAmount, megaAmount);
    }
  }
  ...
  function finalizeRedeem(uint256 redeemIndex) external nonReentrant validateRedeem(msg.sender, redeemIndex) {
    ...
    balance.redeemingAmount = balance.redeemingAmount.sub(_redeem.ESMEGAAmount);
    _finalizeRedeem(msg.sender, _redeem.ESMEGAAmount, _redeem.megaAmount);

    // handle dividends compensation if any was active
    if(_redeem.dividendsAllocation > 0) {
      // deallocate from dividends
      IESMEGATokenUsage(_redeem.dividendsAddress).deallocate(msg.sender, _redeem.dividendsAllocation, new bytes(0));
    }
  ...
  function cancelRedeem(uint256 redeemIndex) external nonReentrant validateRedeem(msg.sender, redeemIndex) {
    ...
    // make redeeming ESMEGA available again
    balance.redeemingAmount = balance.redeemingAmount.sub(_redeem.ESMEGAAmount);
    _transfer(address(this), msg.sender, _redeem.ESMEGAAmount);
    ...
    emit CancelRedeem(msg.sender, _redeem.ESMEGAAmount);
  ...
  /**
   * @dev Allocates caller's "amount" of available ESMEGA to "usageAddress" contract
   *
  ...
  function allocate(address usageAddress, uint256 amount, bytes calldata usageData) external nonReentrant {
    ...
    // allocates ESMEGA to usageContract
    IESMEGATokenUsage(usageAddress).allocate(msg.sender, amount, usageData);
  }

  /**
   * @dev Allocates "amount" of available ESMEGA from "userAddress" to caller (ie usage contract)
   *
   * Caller must have an allocation approval for the required ESMEGA ESMEGA from "userAddress"
   */
  ...
  /**
   * @dev Deallocates caller's "amount" of available ESMEGA from "usageAddress" contract
   *
   * args specific to usage contract must be passed into "usageData"
   */
  function deallocate(address usageAddress, uint256 amount, bytes calldata usageData) external nonReentrant {
    ...
    // deallocate ESMEGA into usageContract
    IESMEGATokenUsage(usageAddress).deallocate(msg.sender, amount, usageData);
  }

  /**
   * @dev Deallocates "amount" of allocated ESMEGA belonging to "userAddress" from caller (ie usage contract)
   *
   * Caller can only deallocate ESMEGA from itself
   */
  ...
  /**
   * @dev Convert caller's "amount" of mega into ESMEGA to "to"
   */
  function _convert(uint256 amount, address to) internal {
    ...
    megaToken.safeTransferFrom(msg.sender, address(this), amount);
  }

  /**
   * @dev Finalizes the redeeming process for "userAddress" by transferring him "megaAmount" and removing "ESMEGAAmount" from supply
   *
   * Any vesting check should be ran before calling this
   * mega excess is automatically burnt
   */
  function _finalizeRedeem(address userAddress, uint256 ESMEGAAmount, uint256 megaAmount) internal {
    uint256 megaExcess = ESMEGAAmount.sub(megaAmount);

    // sends due mega tokens
    megaToken.safeTransfer(userAddress, megaAmount);

    // burns mega excess if any
    megaToken.burn(megaExcess);
    _burn(address(this), ESMEGAAmount);

    emit FinalizeRedeem(userAddress, ESMEGAAmount, megaAmount);
  }

  /**
   * @dev Allocates "userAddress" user's "amount" of available ESMEGA to "usageAddress" contract
   *
   */
  function _allocate(address userAddress, address usageAddress, uint256 amount) internal {
    ...
    uint256 approvedESMEGA = usageApprovals[userAddress][usageAddress];
    require(approvedESMEGA >= amount, "allocate: non authorized amount");

    // remove allocated amount from usage's approved amount
    usageApprovals[userAddress][usageAddress] = approvedESMEGA.sub(amount);
  ...
  /**
   * @dev Deallocates "amount" of available ESMEGA to "usageAddress" contract
   *
   * args specific to usage contract must be passed into "usageData"
   */
  function _deallocate(address userAddress, address usageAddress, uint256 amount) internal {
    ...
    // check if there is enough allocated ESMEGA to this usage to deallocate
    ...
    // burn corresponding mega and ESMEGA
    megaToken.burn(deallocationFeeAmount);
